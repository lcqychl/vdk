...
import (
	...
	"github.com/lcqychl/vdk/codec"
	"github.com/lcqychl/vdk/codec/aacparser"
	"github.com/lcqychl/vdk/codec/h264parser"
	"github.com/lcqychl/vdk/codec/h265parser"
	"github.com/lcqychl/vdk/format/rtsp/sdp"
)
...
func Dial(options RTSPClientOptions) (*RTSPClient, error) {
	...
	for _, i2 := range client.mediaSDP {
		...
		if i2.AVType == VIDEO {
			if i2.Type == av.H264 {
				if len(i2.SpropParameterSets) > 1 && len(i2.SpropParameterSets[0]) > 3 {
		...
		for _, nal := range nalRaw {
			if client.videoCodec == av.H265 {
				...
				case naluType == 8:
					client.CodecUpdatePPS(nal)
				case naluType == 24:
					client.Println("24 Type need add next version report https://github.com/lcqychl/vdk")
				case naluType == 28:
